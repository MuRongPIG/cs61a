...
def interleaved_sum(n, f_odd, f_even):
    ...
    def sum(n,p, f_odd, f_even):
        if p > n :return 0
        if  p == n :return f_odd(p)
        return f_odd(p) + f_even(p+1) + sum(n,p+2,f_odd,f_even)
    return sum(n,1,f_odd,f_even)
