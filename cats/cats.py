...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    return [i for i in paragraphs if select(i)][k] if len([i for i in paragraphs if select(i)]) >= k+1 else ''
    # END PROBLEM 1
...
def about(keywords):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return lambda x: True if [i for i in keywords if i in [y for y in remove_punctuation(x.lower()).split()]] else False 
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if not typed_words or not source_words:return 0.0
    count = 0
    compare_length = min(len(typed_words),len(source_words))
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        acc_list = []
        for i in word_list:
            acc_list.append(diff_function(typed_word,i,limit))
        return word_list[acc_list.index(min(acc_list))] if [i for i in acc_list if i <= limit] else typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if typed == source:return 0 # 完全相同

    if not typed or not source: # 长度不一致
        return max(len(typed),len(source))
    
    if typed and source and limit == 0: # limit用尽
        return 1
...
def minimum_mewtations(typed, source, limit): # 不会写
    ...
    if abs(len(typed) - len(source)) > limit:
        ...
        add = (
            1 + minimum_mewtations((source[0] + typed)[1:], source[1:], limit - 1)
            if typed[0] != source[0]
            else minimum_mewtations(typed[1:], source[1:], limit)
        )  # Fill in these lines
        remove = (
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[a[i+1] - a[i] for i in range(len(a) - 1)] for a in tpp]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    temp = [[] for q in word_indices]
    res = []
    result = [[] for q in player_indices]
    for i in player_indices:
        for j in word_indices:
            temp[j].append(get_time(times,i,j))
    
    for y in temp:
        res.append(y.index(min(y)))

    for m in range(len(res)):
        result[res[m]].append(words[m])
